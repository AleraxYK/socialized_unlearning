...
def baseline_retrain():
    ...
    for ep in range(1, hyperparams["epochs"] + 1):
        ...
        print(
            f"[Retrain] epoch {ep:02d}/{hyperparams['epochs']} "
    ...
    torch.save(model.state_dict(), "results/checkpoints/baseline_retrain.pth")
    print("\nSalvato: results/checkpoints/baseline_retrain.pth")

    with open("results/reports/baseline_retrain_curve.json", "w") as f:
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("Baseline Retrain — Retain vs Forget Accuracy")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("results/plots/baseline_retrain_acc.png", dpi=400, bbox_inches="tight")
    plt.close()
    print("Salvato: results/plots/baseline_retrain_acc.png")
